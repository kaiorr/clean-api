...
describe('SingUp Controller', () => {
  ...
  test('shoul return 400 if password confirmation fails ', async () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'any_name',
  ...
  test('shoul return 400 if no passwordConfirm is provided ', async () => {
    const { sut } = makeSut()
    const httpRequest = {
      body: {
        name: 'any_name',
  ...
  test('shoul call AddAccount with correct values ', async () => {
